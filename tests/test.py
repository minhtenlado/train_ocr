"""
End-to-End Pipeline Test Script (YOLO Detection + SquareCRNN OCR Inference).
"""
...
def main():
    ...
    ocr_model_path = os.path.join(base_dir, 'models', 'best_square_ocr_pro.pth')

    images_dir = os.path.join(base_dir, 'images')
    image_path = None
    if os.path.exists(images_dir):
        for file in sorted(os.listdir(images_dir)):
            if file.lower().endswith(('.jpg', '.jpeg', '.png')) and 'pipeline' not in file.lower():
                image_path = os.path.join(images_dir, file)
                break

    if image_path is None:
        image_path = os.path.join(base_dir, 'images', 'img_215.jpg')
    ...
    output_preview_path = os.path.join(base_dir, 'pipeline_output_preview.jpg')
    cv2.imwrite(output_preview_path, img_bgr)
    print(f"[+] Output preview saved to: {output_preview_path}")

    # Display GUI window if DISPLAY environment variable is set
